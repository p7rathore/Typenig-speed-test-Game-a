...
let typint_test = document.querySelector(".content-box p");
let inputfild = document.querySelector(".contener .input-fild");
let mistakeTag = document.querySelector(".mistakeTag span")
let changeT = document.querySelector(".changeT span b");
...
function randomParagraf(){
    let random = Math.floor(Math.random()*paragraphs.length)
    typint_test.innerHTML = "" ;
  paragraphs[random].split("").forEach(span =>{
   let spanTag = `<span>${span}</span>`;
   typint_test.innerHTML += spanTag ;
  });
  // focus inpute fild on keydown or click event
  document.addEventListener("keydown", () => inputfild.focus());
  typint_test.addEventListener("click", () => inputfild.focus());


}

function iniTyping(){
    const charectors = typint_test.querySelectorAll("span");
    let typeChar = inputfild.value.split("")[charIndex];
    if(charIndex < charectors.length -1 && timeLeft > 0){
        if(!isTyping) {
        ...
        if (typeChar == null) {
            charIndex--;
            if (charectors[charIndex].classList.contains("incorrect")) {
                mistekIndex--;
            }
            charectors[charIndex].classList.remove("incorrect","incorrect")
        } else {
            if (charectors[charIndex].innerText === typeChar) {
                // if typing charect is correct 
                charectors[charIndex].classList.add("correct")
            } else {
             mistekIndex++;
             charectors[charIndex].classList.add("incorrect")
            }
            charIndex++;
    }
  }else{
    ...
    mistakeTag.innerHTML = mistekIndex;
    let WPMTag = Math.round((((charIndex-mistekIndex)/5)/(maxTime-timeLeft))*60);
    WPM = WPM < 0 || !WPM ||WPM === Infinity ? 0 : WPM ;
    CPM.innerHTML =  charIndex - mistekIndex;
...
randomParagraf();
inputfild.addEventListener("input",iniTyping)
